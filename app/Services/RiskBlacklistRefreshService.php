...
abstract class RiskBlacklistRefreshService
{
    protected const SNAPSHOT_VERSION = 1;
     ...
     * 注入可测试时钟和固定字段日志记录器。
     */
    public function __construct(?callable $clock = null, ?callable $logRecorder = null)
    {
    ...
    /**
     * 刷新全部订阅并在同一把锁内发布完整快照和状态。
     */
    public function refresh(?string $urlList = null): array
    {
        $configuredValue = $urlList === null ? $this->configuredUrlValue() : $urlList;
        $urls = $this->normalizeSubscriptionUrls($configuredValue);
        $lock = Cache::lock(
            CacheKey::get($this->refreshLockCacheKey(), 'current'),
     ...
     * 在锁内复查开关和关闭标记后执行定时刷新。
     */
    public function refreshScheduled(): ?array
    {
        $urls = $this->normalizeSubscriptionUrls($this->configuredUrlValue());
        $lock = Cache::lock(
            CacheKey::get($this->refreshLockCacheKey(), 'current'),
            $this->lockLeaseSeconds(count($urls))
        );
        ...
        return $lock->block(self::LOCK_WAIT_SECONDS, function () use ($urls): ?array {
            if (!$this->enabledConfigValue()
                || Cache::get(CacheKey::get($this->disabledMarkerCacheKey(), 'current'))) {
                return null;
            }
    ...
    /**
     * 先设置关闭标记，再在刷新锁内清理本域全部规则快照。
     */
    public function disableAndClearSnapshots(): void
    {
        $markerKey = CacheKey::get($this->disabledMarkerCacheKey(), 'current');
        ...
        $lock->block(self::LOCK_WAIT_SECONDS, function (): void {
            $this->clearRuleSnapshotsLocked();
        });
    }

    /**
     * 在刷新锁内移除关闭标记并使用已持久化地址重新刷新。
     */
    public function refreshAfterEnable(string $urlList): array
    {
        ...
        return $lock->block(self::LOCK_WAIT_SECONDS, function () use ($urls): array {
            Cache::forget(CacheKey::get($this->disabledMarkerCacheKey(), 'current'));
            try {
                return $this->refreshLocked($urls);
            } catch (\Throwable $exception) {
                return $this->recordUnexpectedFailureLocked($urls);
    ...
    /**
     * 返回字段稳定且仅包含脱敏来源的最新刷新状态。
     */
    public function getLatestStatus(): array
    {
        $cached = Cache::get(CacheKey::get($this->refreshStatusCacheKey(), 'current'));
        ...
        return [
            ...
            'enabled' => $this->enabledConfigValue(),
            ...
            'rule_count' => $this->enabledConfigValue()
                ? max(0, (int)($cached['rule_count'] ?? 0))
                : 0,
     ...
     * 返回当前子系统持久化的订阅地址值。
     */
    abstract protected function configuredUrlValue();

    /**
     * 返回当前子系统的持久化启用状态。
     */
    abstract protected function enabledConfigValue(): bool;

    /**
     * 将远程正文解析为公共刷新结果结构。
     */
    abstract protected function parseRemoteBody(string $body): array;

    /**
     * 规范化跨来源合并后的规则数组。
     */
    abstract protected function normalizeCombinedRules(array $rules): array;

    /**
     * 返回分类器可见快照的缓存族名称。
     */
    abstract protected function snapshotCacheKey(): string;

    /**
     * 返回单源快照的缓存族名称。
     */
    abstract protected function sourceCacheKey(): string;

    /**
     * 返回活动来源索引的缓存族名称。
     */
    abstract protected function sourcesCacheKey(): string;

    /**
     * 返回最新刷新状态的缓存族名称。
     */
    abstract protected function refreshStatusCacheKey(): string;

    /**
     * 返回刷新互斥锁的缓存族名称。
     */
    abstract protected function refreshLockCacheKey(): string;

    /**
     * 返回关闭刷新标记的缓存族名称。
     */
    abstract protected function disabledMarkerCacheKey(): string;
     ...
     * 返回固定系统日志 URI。
     */
    abstract protected function logUri(): string;

    /**
     * 生成不携带远程内容的失败结果。
     */
    protected function failedFetch(string $category, bool $retryable): array
    {
        return [
            'success' => false,
            'retryable' => $retryable,
    ...
    private function refreshLocked(array $urls): array
    {
        ...
        $entries = [];
        foreach ($urls as $url) {
            $entries[] = ['url' => $url, 'hash' => hash('sha256', $url)];
        }

        $activeHashes = array_column($entries, 'hash');
        $previousHashes = Cache::get(CacheKey::get($this->sourcesCacheKey(), 'current'), []);
        ...
        foreach (array_diff($previousHashes, $activeHashes) as $removedHash) {
            if (is_string($removedHash)) {
                Cache::forget(CacheKey::get($this->sourceCacheKey(), $removedHash));
            }
        }
        Cache::forever(CacheKey::get($this->sourcesCacheKey(), 'current'), $activeHashes);

        if ($entries === []) {
            $status = $this->makeStatus('not_configured', $startedAt, 0, 0, 0, 0, 0, 0, []);
            Cache::forever(CacheKey::get($this->refreshStatusCacheKey(), 'current'), $status);
        ...
        foreach ($entries as $entry) {
            ...
            $sourceKey = CacheKey::get($this->sourceCacheKey(), $entry['hash']);
            if ($result['success']) {
    ...
    /**
     * 在刷新锁内删除本域合并、单源和来源索引快照。
     */
    private function clearRuleSnapshotsLocked(): void
    {
        $sourceIndexKey = CacheKey::get($this->sourcesCacheKey(), 'current');
        $sourceHashes = Cache::get($sourceIndexKey, []);
        $sourceHashes = is_array($sourceHashes) ? $sourceHashes : [];
        foreach ($sourceHashes as $sourceHash) {
            if (is_string($sourceHash)) {
                Cache::forget(CacheKey::get($this->sourceCacheKey(), $sourceHash));
            }
        }

        Cache::forget(CacheKey::get($this->snapshotCacheKey(), 'current'));
        Cache::forget($sourceIndexKey);
    }
    ...
    private function makeUnexpectedFailureStatus(array $urls): array
    {
        ...
        $snapshot = Cache::get(CacheKey::get($this->snapshotCacheKey(), 'current'));
    ...
    private function fetchAttempt(string $url, float $attemptDeadline): array
    {
        ...
        while (true) {
            ...
            try {
                ...
                if ($this->exceptionChainContains($exception, 'response_too_large')) {
                ...
                $category = $this->exceptionChainContains($exception, 'response_too_large')
                    ? 'response_too_large'
                    : 'connection';
            ...
            if ($status >= 300 && $status < 400) {
                ...
                $redirectUrl = $this->resolveRedirectUrl($currentUrl, $response->header('Location'));
                if ($redirectUrl === null) {
                    return $this->failedFetch('redirect', false);
                }
            ...
            return $this->parseRemoteBody($body);
        }
    }
    ...
    private function publishCombinedSnapshot(array $sourceHashes, int $updatedAt): array
    {
        ...
        $rules = $this->normalizeCombinedRules($allRules);
        Cache::forever(CacheKey::get($this->snapshotCacheKey(), 'current'), [
            'version' => self::SNAPSHOT_VERSION,
            'rules' => $rules,
    ...
    {
        $snapshot = Cache::get(CacheKey::get($this->sourceCacheKey(), $sourceHash));
        ...
        return $this->normalizeCombinedRules($snapshot['rules']);
    }
    ...
    /**
     * 检查传输异常链中是否包含固定内部错误标记。
     */
    private function exceptionChainContains(\Throwable $exception, string $message): bool
    {
        do {
     ...
     */
    protected function lockLeaseSeconds(int $sourceCount): int
    {
        return max(
